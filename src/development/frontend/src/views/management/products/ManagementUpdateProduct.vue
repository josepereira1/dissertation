<template>
    ...
    <!--<h2 id='top' v-if="product && product != null" class="d-flex align-content-start">UPDATE PRODUCT MANAGEMENT: <span style="color: grey">ID#{{  product.id  }}</span></h2>-->
    <!--<h2 v-else class="d-flex align-content-start">UPDATE PRODUCT MANAGEMENT</h2>-->
    <div v-if="product">
        <div class="container">
            <button v-on:click="updateProduct" type="button" class="btn btn-primary">UPDATE PRODUCT</button>
            ...
            <div class="form-floating">
                <textarea class="my-2 form-control" placeholder="" id="floatingTextarea" v-model="product.id" disabled></textarea>
            ...
            <div class="form-floating">
                <textarea class="my-2 form-control" placeholder="Leave a comment here" id="floatingTextarea" v-model="thumbnailFilename"></textarea>
                <label for="floatingTextarea">Card thumbnail image name</label>
            </div>
            ...
            <ul class="list-group" style="width: 100%;">
                <li class="list-group-item">{{ product.links.thumbnails.split('/').at(-1).split('?')[0] }}</li>
            </ul>

            <div class="line2 my-3"></div>
            
            <div class="form-floating">
                <textarea class="my-2 form-control" placeholder="Leave a comment here" id="floatingTextarea" v-model="carouselFilename"></textarea>
                <label for="floatingTextarea">Carousel image name</label>
            </div>
            ...
            <h6>ADD NEW SPEC:</h6>
            <div class="form-floating">
                <textarea class="form-control" placeholder="Leave a comment here" id="floatingTextarea" v-model="specName"></textarea>
                <label for="floatingTextarea">Label Spec:</label>
            </div>
            <div class="form-floating">
                <textarea class="form-control" placeholder="Leave a comment here" id="floatingTextarea" v-model="specText"></textarea>
            ...
            <div class="form-floating">
                <textarea class="form-control" placeholder="Leave a comment here" id="floatingTextarea" v-model="detailTitle"></textarea>
                <label for="floatingTextarea">Detail title:</label>
            </div>
            <div class="form-floating">
                <textarea class="form-control" placeholder="Leave a comment here" id="floatingTextarea" v-model="detailText"></textarea>
            ...
            <div v-for="paragraph in product.longDetails.paragraphs" v-bind:key="paragraph">
                ...
                <div class="row">
                    <div class="col-11 form-floating">
                        <textarea class="form-control" placeholder="Leave a comment here" id="floatingTextarea" v-model="paragraph.title"></textarea>
                ...
                <div class="form-floating">
                    <textarea class="form-control" placeholder="Leave a comment here" id="floatingTextarea2" style="height: 100px" v-model="paragraph.text"></textarea>
...
export default {
    ...
    async mounted(){
        auth(this)
        const options = {
            url: process.env.VUE_APP_BACKEND_URL + '/api/products/' + this.id,
            method: 'GET',
            headers: {
        ...
        await axios(options).then(response => this.product = response.data)
    },
    methods: {
        async addStock(){
            auth(this)
        ...
        removeFromAddCategories(){
            if(this.removeFromAddCategoryId && this.removeFromAddCategoryId != null && this.addCategories.includes(this.removeFromAddCategoryId)){
                for( var i = 0; i < this.addCategories.length; i++){ 
                    if (this.addCategories[i] === this.removeFromAddCategoryId) { 
        ...
        async updateProduct(){
            ...
            if(this.product){
                ...
                const options = {
                    url: process.env.VUE_APP_BACKEND_URL + '/api/processes?id=update.product.saga.0',
                    method: 'PUT',
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type': 'application/json;charset=UTF-8',
                ...
                alert('The update product processe started successfully, check the processes section to check the result.')
            }
        }
