                ...
                <div v-for="i in lines" v-bind:key="i">
                    <div v-if="category.products.length >= 1" class="d-flex justify-content-center" style="display: flex; width: 100%">
                        <div id="product" v-for="product in category.products.slice(productsPerLine*(i-1),productsPerLine*i)" v-bind:key="product.id" class="my-card">
                            <router-link v-bind:to="{ name: 'ProductDetails', params: { id: product.id}}">
                ...
                <nav class="my-3 d-flex justify-content-center" aria-label="Page navigation example">
                    <ul class="pagination">
                        <li v-for="i in pages" v-bind:key="i" class="page-item">
                            <div v-if="i == page">
                                <router-link v-bind:to="{ name: 'Products', params: {id: category.id}, query: {page: i}}"><div class="page-link" style="background: grey;color: white;">{{  i  }}</div></router-link>
                            </div>
                            <div v-else>
                                <router-link v-bind:to="{ name: 'Products', params: {id: category.id}, query: {page: i}}"><div class="page-link">{{  i  }}</div></router-link>
                            </div>
                        </li>
                    </ul>
                </nav>
...
export default {
    ...
    async mounted(){
        ...
        if(this.categoryId 
            ...
            await fetch(process.env.VUE_APP_BACKEND_URL + '/api/categories/' + this.categoryId + '/products/visible?page=' + this.page + '&products_per_page=' + (this.productsPerLine * this.lines))
                .then(res => res.json())
