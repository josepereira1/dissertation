package com.ecommerce.qcap.inbound.messaging.cqrs.category.category.createproductincategories.mapper.in;
...
public interface IProductInMapper {
    ...
    static CreateProductInCategoriesDTO toCreateProductInCategoriesDTO(UpdateProductInCategoriesDTO product, Collection<Long> currentCategoryIds){
        CreateProductInCategoriesDTO tmp1 = new CreateProductInCategoriesDTO();
        tmp1.setId(product.getId());
        tmp1.setName(product.getName());
        tmp1.setShortDetails(product.getShortDetails());
        tmp1.setCurrency(product.getCurrency());
        ...
        tmp1.setOwner(product.getOwner());
        tmp1.setCountermeasure(product.getCountermeasure());
        tmp1.setCategories(currentCategoryIds);
        return tmp1;
    }
}
